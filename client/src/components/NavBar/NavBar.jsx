...
import { logoutPromise } from "../../helpers/authHelpers";
...
const NavBar = () => {
  ...
  const openMessage = () => {
    let loading = new Promise((res, rej) => {
      const key = "updatable";
      message.loading({ content: "Cerrando sesion...", key });
      setTimeout(() => {
        message.success({ content: "Logout exitoso", key, duration: 2 });
        res(true);
      }, 1000);
    });
    return loading;
  ...
  const warning = () => {
    message.warning("Error al intentar cerrar sesion, contacte con soporte");
  };

  const onSingOut = () => {
    logoutPromise(dispatch).then((res) => {
      if (res) {
    ...
    <div className="navbar">
      ...
      <div className="navbar--topcenter">
        <ul className="topList">
          ...
          <div onClick={onSingOut} className="link">
            {ath && <li className="topList--item">LOGOUT</li>}
          </div>
        </ul>
      </div>
